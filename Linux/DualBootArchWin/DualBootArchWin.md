...
- ## Installer un environnement de bureau
  ...
  pacman -S xorg
  ```
  Il faut ensuite installer un gestionnaire d'affichage permettant de démarrer un environnement de bureau. Ce n'est pas réellement nécessaire mais démarrer depuis un terminal est bien moins intuitif que démarrer avec une interface graphique. Des gestionnaires d'affichage il y en a plusieurs. Je conseille LightDM (avec le greeter GTK) ou SDDM (avec le thème breeze).
  LightDM :
  ```bash
