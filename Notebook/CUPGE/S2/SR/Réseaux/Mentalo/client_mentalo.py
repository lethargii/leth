...
adress = input("Enter the IP adress of the server :")
# Se connecter au jeu
clientsocket.sendto(f"{BALISE_NEW_PLAYER}{input('Enter your name : ')}".encode("utf-8"),(adress, 8080))
...
def send():
    ...
    while True:
        ...
        # Envoyer le message
        if message != "":
            clientsocket.sendto(message.encode("utf-8"), (adress, 8080))
